        ...
        (err, card) => {
            card.checklists.id(idChecklist).items.forEach((item) => {
                if (item._id.equals(id)){
                    if ('undefined' !== typeof req.body.title) item.title = req.body.title
                    if ('undefined' !== typeof req.body.isDone) item.isDone = req.body.isDone
                }
            })
            card.save(
                (err) => {
                    if (err) res.status(401).send(err)
                    else res.status(200).send("Successfully updated the list of id " + id)
                }
            )
        }
    )
